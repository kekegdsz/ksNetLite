...
- GitHub Release URL (replace with your real repo):
  `https://github.com/kekegdsz/ksNetLite/releases/latest/download/ksnetlite-debug.apk`

### Download QR Code

> QR currently points to the release URL template above. Replace `kekegdsz/ksNetLite` in README and regenerate if needed.

![APK Download QR](./apk/download-qr.png)
...
如果本机没有 `gradlew`，也可以使用本地 Gradle：

```bash
gradle -p . installDebug
```
